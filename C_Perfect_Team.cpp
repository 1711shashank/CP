...
void solve(){
    ...
    x+= d;                      //shifting the extra student to x


                                // revaulting the value of c,m after shifting 
    if( c > m){
        c-=d;                   
    }
    else {
    ...
    ll mn = min({c,m,x});           // min team 
                                    // finding the left stuent
    c-=mn;                          
    m-=mn;
    x-=mn;
                                    // finding the number of team which can be formed by the remiming student
    mn += (c+m)/3;
